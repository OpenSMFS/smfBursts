...
_ph_sel_color_dict = {Ph_sel('all'): blue, Ph_sel('0ex0em'): green,
                      Ph_sel('0ex1em'): red, Ph_sel('1ex1em'): purple,
                      Ph_sel('1ex0em'): 'c', }
_ph_sel_label_dict = {Ph_sel('all'): 'All-ph', Ph_sel('0ex0em'): 'DexDem',
                      Ph_sel('0ex1em'): 'DexAem', Ph_sel('1ex1em'): 'AexAem',
                      Ph_sel('1ex0em'): 'AexDem'}
...
def timetrace(d, i=0, binwidth=1e-3, bins=None, tmin=0, tmax=200,
    ...
    for ix, (ph_sel, invert) in enumerate(zip(ph_sel_list, invert_list)):
        if not mask_empty(d.get_ph_mask(i, ph_sel=ph_sel)):
            timetrace_single(
...
def ratetrace_single(d, i=0, m=None, max_num_ph=1e6, tmin=0, tmax=200,
    ...
    rates = 1e-3 * mtuple_rates(ph_times, m) / d.clk_p
    if invert:
        rates *= -1
    times = mtuple_rates_t(ph_times, m) * d.clk_p
...
def hist_interphoton(d, i=0, binwidth=1e-4, tmax=None, bins=None, period=None,
    ...
    for ix, ph_sel in enumerate(ph_sel_list):
        ...
        if not np.all([mask_empty(d.get_ph_mask(j, ph_sel=ph_sel)) for j in sl]):
