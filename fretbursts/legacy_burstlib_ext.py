...
from .datamodel.tables import Param
from .photondata import Burst
...
def calc_mean_lifetime(data:Data, t1:int=0, t2=np.inf, ph_sel:Ph_sel=Ph_sel('0ex0em'))->tuple[np.ndarray[np.double],...]:
    ...
    """
    for origin in data._data:
        if ph_sel not in origin.irf_thresh:
            origin.irf_thresh[ph_sel] = t1
    return tuple(burst_table['nanomean', ph_sel] for burst_table in data._burst_tables)
    # mean_lifetimes = []
...
def calc_bg_brute_cache(dx, min_ph_delay_list=None, return_all=False,
    ...
    """
    raise DeprecationWarning("This function is deprecated, optimized with new Param system")
    ...
    # comp_filter = tb.Filters(complevel=6, complib='zlib')
    # store = tb.open_file(store_fname, mode='a', filters=comp_filter)
    # loaded = False
...
def burst_data(dx:Data, include_bg=False, include_ph_index=False,
               skip_ch=None):
...
def burst_photons(dx:Data, skip_ch=None):
    ...
    """
    assert dx.nch == 1, 'Multispot photon-data export not implemented.'
    ich = 0
...
def fit_bursts_kde_peak(dx:Data, burst_data:str='E', bandwidth:float=0.03, weights=None,
                        gamma:float=1.0, add_naa=False, x_range:tuple[float,float]=(-0.1, 1.1),
...
def calc_mdelays_hist(d:Data, ich=0, m=10, period=(0, -1), bins_s=(0, 10, 0.02),
...
def join_data(d_list:Sequence[Data,...], gap=0):
    """Joins burst data of different measurements in a single `Data` object.
    
    .. note::
        
        This method is now just a wrapper for :func:`legacy_burstlib_ext.group_data`, 
        old use of :func:`legacy_burstlib_ext.join_data` is deprecated.
    ...
    single multispot acquisition).
    
    .. note::
        
        Use of this function is discouraged. If you need to join data, use
        the new :class:`datamodel.tables.DataSet` system.
    
    Arguments:
        d_list (list of Data objects): the list of measurements to join.
        gap (float): **Ignored** kept for backwards compatibility.

    Returns:
        A `Data` object containing bursts from the all the objects in `d_list`.

    """
    return group_data(d_list)
...
def group_data(d_list):
    ...
    for attr in Data._copyattrs:
        if hasattr(d_list[0], attr):
            setattr(out, attr, getattr(d_list[0], attr))
            if isinstance(getattr(out, attr), tuple):
                for data in d_list[1:]:
                    setattr(out, attr, getattr(out, attr)+getattr(data, attr))
    return out


def burst_search_and_gate(dx, F=6, m=10, min_rate_cps=None, c=-1,
                          ph_sel1=Ph_sel('0ex'),
                          ph_sel2=Ph_sel('1ex1em'), compact=False, mute=False):
    ...
    Arguments:
        ...
        min_rate_cps (float or tuple): **deprecated** FRETBUrsts no longer
            supports 
    ...
    """
    dout = dx.copy()
    dout.delete_burst_data()
    params = dict(channels=(ph_sel1, ph_sel2), m=m, F=F, c=c)
    dout._burst_params = tuple(Param(Burst, params, {'bg':bg_param}) 
                               for bg_param in dout._bg_params)
    dout._burst_tables = tuple(data.get_table(burst_param) for data, burst_param 
                               in zip(dout._data, dout._burst_params))
    return dout
...
#  Burst asymmetry
#
def get_burst_photons(data:Data, ich:int=0, ph_sel:Ph_sel=Ph_sel('all'))->np.ndarray[np.object_]:
    ...
    """
    return data._bg_tables[ich]['ph_times', ph_sel]
...
def asymmetry(dx, ich=0, func=np.mean, dropnan=True):
    ...
    """
    if ich is None:
        return [asymmetry(dx, ich=i, func=func, dropnan=dropnan) for i in range(dx.nch)]
