...
"""
.. currentmodule:: fretbursts.datamodel.immutable
...
from .utils import (_DataLike, _ImDataLike, ImDict, tupledict, FixedDict, 
    _dimscompare, _echo, iter_funcinput, _ndarray_to_str, _tuple_array, _eq, 
...
class TypeValidator(_ImDataLike):
    ...
    --------------
    type_ : type
        The type of data the ``TypevVlidator`` object expects
    check : callable
    ...
    def check_val(self, val:Any, *predata, **kwargs):
        ...
        ----------
        val : Any
            Value should be subtype of TypeValidator type_ attribute.
        *predata : 
    ...
    def convert_type(cls, type_:Union[type,"TypeValidator"])->"TypeValidator":
        ...
        ------
        ValueError
            type_ does not have a corresponding TypeValidator.
    ...
    @classmethod
    def _get_subgroup(cls, dct, args, prev=None):
    ...
    @classmethod
    def get_subgroup(cls, *args:Hashable):
        """
        Get the dictionay/TypeValidator/type
    ...
    @classmethod
    def _recurse_subgroups(cls, dct, final, args, curgroup=None):
    ...
    @classmethod
    def _init_subgroup(cls, dct, key, args, curgroup):
        if len(args) != 1:
    ...
    @classmethod
    def _init_subclass(cls, dct, key, args, curgroup):
        if len(args) != 1:
    ...
    def read_any(cls, group:tb.Node)->Any:
        """
        Read HDF5 group as python object. Inspects the title, uses __call__ key
...
def _check_num(supertype:type, outtype:type, val:Number, mn:Number=-np.inf, mx:Number=np.inf, 
               isin:Union[tuple[Number,...],list[Number],set[Number]]=None, **kwargs)->int:
    """
...
def check_arbsequence(type_:type, typecall:Callable[[Sequence],Sequence], vals:Sequence, 
    ...
    typecall : Callable[[Sequence],Sequence]
        Callable that takes val and converts to type, often same as type_.
    vals : Sequence
    ...
    **kwargs : Any
    ...
    TypeError
        type_ is not hashable, and therefore invalid for storing with TypeValidator.

    Returns
    -------
    Sequence
        val converted to type_ by typecall.
...
def read_arbsequence(group:tb.Group, dct:dict, tp:str)->Sequence:
    ...
    **For use with :class:`TypeValidator`**
    ...
    dct : dict
        dictionary containing mapping of values in SequenceType_ to function
...
def _get_tupledictargs(val:Any)->Union[tupledict,Sequence[tuple[str,Any]]]:
...
def _tupledict_fromany(val:Union[tuple,dict,tupledict])->tupledict:
...
def node_read_tupledict(val:str)->str:
...
# TV_tupledict = TypeValidator(tupledict, check_tupledict, write_tupledict, read_tupledict)
...
class _ImData(_DataLike):
    ...
    There are a number of key class variables used to control behavior::
        ...
        - ``_typeconversions`` a :class:`fretbursts.utils.misc.ImDict` with keys
    ...
    def _replace_fields(self, fields:dict[str,Any]=None,  pop:tuple[str]=None, _strict=True)->"_ImData":
    ...
    def _get_typeconversions(self, k, val):
