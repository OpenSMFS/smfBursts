...
def colorcategory(*args:np.ndarray, cmap:str|mpl.colors.Colormap=None, reduce:bool=False, **kwargs)->dict[str:Any]:
    """
...
def gaus_2Dkde_cmap(datax:np.ndarray, datay:np.ndarray, outx:np.ndarray, outy:np.ndarray, 
    ...
    if not thresh_raw:
            dens[dens<thresh] = np.nan
    print(np.nanmin(dens), np.nanmax(dens), np.nanmean(dens), np.isnan(dens).sum())
    return dens
