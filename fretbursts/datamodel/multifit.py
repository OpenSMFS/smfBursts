...
.. |Param| replace:: `Column <fretbursts.datamodel.tables.Param>`
.. |Column| replace:: `Column <fretbursts.datamodel.tables.Column>`
.. |GateGroup| replace:: `Column <fretbursts.datamodel.tables.GateGroup>`
...
def value_lsq(params:np.ndarray, x:np.ndarray, y:np.ndarray, func:FitFunc, **kwargs)->float:
    ...
    **kwargs : Any
    ...
    """
    return ((y - func(params, x, **kwargs))**2).sum()
...
ngaus_cdfbins = multi_cdfbins(stats.norm.cdf, ['mu', 'simga'], name='normal.pdf')
...
class Hist:
    ...
    def binlocs(self)->np.ndarray[np.float64]:
        ...
        out = np.empty([b.size for b in self._bins]+[len(self._bins)])
        for i, b in enumerate(self._bins):
            out[...,i] = b.reshape(*(1 if j != i else b.size for j in range(len(self._bins))))
...
def fit_hist_cdf(hist:Hist, func:FitFunc, init:np.ndarray, func_kwargs:dict=None, **kwargs)->OptimizeResult:
    """
    Perform least-squares fitting of 1D :class:`Hist` against a multi-distribution.
    Shoudl be a ``n<dist>_cdf`` type distribution.
    ...
    func_kwargs : dict, optional
        Kwargs handed to func. The default is None.
    **kwargs : Any
        Additional kwargs passed to minimize_ .
    ...
    vfunc = value_lsq_by_cdf if func_kwargs is None else partial(value_lsq_by_cdf, **func_kwargs)
    return minimize(vfunc, init, args=(bins, hist.pmf, func), **kwargs)
