...
If writing your own extension to FRETBursts, there are 3 more functions that are
important\:
...
basic usage::
    ...
    frb.register_citation("CleeseNature1970", "Cleese, J. Fluorescent properties of Spam. Nature (1970) 123. 21", 
                          doi="10.1000/nature.123.21", bibtex=".", ris=".")
    
    frb.register_citation("IdleScience1969", "Idle, E. Mortality of Domesticated Mopsitta Tanta. Science. (1969) 322. 55".
                          doi="10.1111/science.322.55", bibtex="..", ris="..")
    
    frb.register_citation("PalinCell1970", "Palin, M. Supression of Diffusivity in SPA15 by Inq. PNAS, (1970) 999, 11",
                          doi="10.1222/pnas.999.11", bibtex="...", ris="...")
    
    frb.create_citation_group("monty", "IdleScience1969", "PalinCell1970")
    
    @frb.cite("IdleScience2027", purpose="dead")
    def norway(*args):
        return 'blue'
    
    def cheese(args):
        if len(args) > 2:
            frb.add_citation("monty", "IdleSciecne2027", purpose="meaning of life")
        return "cheddar"
    for citation in frb.get_citations(include_purpose):
        print(citation)
...
class Citation:
    ...
    def get_style(self, name:str)->Union[str,None]:
        return self._styles.get(name, self.prefered_style)
    
    @property
    def prefered_style(self):
...
def register_citation(tag:str=None, citation:str=None, overwrite:bool=False, 
    ...
    ----------
    tag : str, optional
        Short identifier string used to identify citation internally. FRETBursts
        citations follow the format ``[firstauthor][journalabreviation][year]``. 
        The default is None.
    citation : str, optional
        Full citation (FRETBursts citations given in Nature format) string.
        The default is None.
    overwrite : bool, optional
...
def cite(*args:str, tags:list[str]=None, cite_groups:list[str]=None, citations:list[str]=None, 
    ...
    *args : str
    ...
    citations : list[str], optional
        String of citation as it would appear in a paper. Citations included in FRETBursts use
    ...
    styles : list[dict[str,str]], optional
        ...
        citation. Citations included in FRETBursts include bibtex, ris, and doi formats.
        The default is None.
    purpose : str | set[str], optional
        Purpose descriptor for all citations to add to notes when using :func:`get_citations`
        as ``frb.get_citations(include_purpose=True)``. Note that unlike all other keyword arguments,
...
def set_prefered_style(name:str)->None:
    ...
    ----------
    name : str
        Name of desired style, FRETBursts citations always have ``'bibtex'``
        and ``'ris'`` styles.
