...
if __name__ == '__main__':
    ...
    plt.plot(TT_10.ravel(), lw=1.5, label='P = 10%')
    plt.plot(TT_01.ravel(), lw=1.5, label='P = 1%')
    plt.plot(TT_001.ravel(), lw=1.5, label='P = 0.1%')
    plt.plot(TT_old.ravel(), lw=2, color='k', label='T = m/(2.5*bg_rate)')
    plt.legend(loc='best')
    plt.grid(True)
    plt.xlabel("m = "+str([3,5,10,20,50])+"; rate = "+str([100, 1000, 2000, 5000]))
    plt.ylabel('T (s)')
    plt.title("T selection comparison")
