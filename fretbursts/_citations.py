...
_bibtex = {_doi_bib(cite):f'@{cite}' for cite in 
           files('fretbursts').joinpath('citations/citations.bib').read_text(encoding='utf8').split('@') 
           if cite}
    
_risrefs = {_doi_ris(record):record for record in 
            _split_ris(files('fretbursts').joinpath('citations/citations.ris').read_text(encoding='utf8')) 
            if record}

_citations = json.loads(files('fretbursts').joinpath('citations/citations.json').read_text(encoding='utf8'))
