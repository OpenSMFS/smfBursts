...
from fretbursts.datamodel.immutabledata import (_ImData, TypeValidator, register_PyCode, 
...
global ge_gate, le_gate
ge_gate, le_gate = None, None
...
def test_tupledict():
    ...
    assert td == TV_tdpass.check_val(td)
    with pytest.raises(Exception):
...
global ImDataTest_simple, ImDataTest_defaults, ImDataTest_required, ImDataTest_complete
ImDataTest_simple = None
ImDataTest_defaults = None
ImDataTest_required = None
ImDataTest_complete = None

def test_ImData_attrs():
    """Test attribute retreval from ImData"""
    global ImDataTest_simple
...
@pytest.mark.skipif(ImDataTest_required is None or ImDataTest_defaults is None)
def test_ImData_complete():
    """Test that ImData subclass specifying everything remains functional"""
    global ImDataTest_complete
...
array_ = 'ndarray', np.arange(4).reshape(2,2)
int_ = 'int', 1
float_ = 'float', 1.0
bool_ = 'bool', True
...
pycode_ = None if ge_gate is None else ('py_code', ge_gate)
imsimple_ = None if ImDataTest_simple is None else ('ImDataTest_simple', ImDataTest_simple(4,2.71))
imdefaults_ = None if ImDataTest_defaults is None else ('ImDataTest_defaults', ImDataTest_defaults())
imrequired_ = None if ImDataTest_required is None else ('ImDataTest_required', ImDataTest_required(8,3.14))
imcomplete_ = None if ImDataTest_complete is None else ('ImDataTest_complete', ImDataTest_complete())


@pytest.fixture(params=[array_, int_, float_, str_, tuple_, tupledict_, imsimple_,
                        imdefaults_, imrequired_, imcomplete_])
def all_val_types(request):
    """Fixture to test each type of type validator"""
    if request.param is None:
        pytest.skip("failed to create given type")
    return request.param
