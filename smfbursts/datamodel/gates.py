...
"""
.. currentmodule:: smfbursts.immutabledata.gates
...
def _col_index(col:Column, columns:tuple[Column,...],vec:np.ndarray[np.float64])->float:
    for c, v in zip(columns, vec):
        if c == col:
            return v
    return 0.0


def _comp_ellipse_linaer(gateE:Gate, gateL:Gate)->int:
    if any(c not in gateE.columns for c in gateL.columns):
        return 0b1111
    vec = np.array([_col_index(col, gateL.columns, gateL.params['vec']) 
                    for col in gateE.columns])
    vecprime = vec * (gateL.params['m'] - np.dot(vec, gateE.params['center']))
    intersection = gateE.func(*vecprime.reshape(-1,1), 
                              transform=gateE.params['transform'], 
                              center=np.zeros(vec.shape))[0]
    if intersection:
        return 0b1111
    center = gateL.func(*gateE.params['center'].reshape(-1,1), m=gateL.params['m'], vec=vec)[0]
    return 0b1101 if center else 0b0111
...
GateDefinition.set_gate_comparison(ELLIPSOID_LT_gate, LIN_GT_gate, _comp_ellipse_linaer)
GateDefinition.set_gate_comparison(ELLIPSOID_LEQ_gate, LIN_GT_gate, _comp_ellipse_linaer)
GateDefinition.set_gate_comparison(ELLIPSOID_LT_gate, LIN_GEQ_gate, _comp_ellipse_linaer)
GateDefinition.set_gate_comparison(ELLIPSOID_LEQ_gate, LIN_GEQ_gate, _comp_ellipse_linaer)
