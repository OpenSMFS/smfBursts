...
"""
The ``smfbursts.bursttables`` module defines base :class:`Bursts` table that 
...
class Bursts(BasePhotonTable):
    ...
    ------
        streams : tuple[PhSel,...]
            tuple of :class:`smfbursts.ph_sel.PhSel` objects, 1 per burst search.
    ...
    Columns
    -------
    Uses :class:`smfbursts.photondata.BasePhotonTable` columns.
    ...
    def __init_columns__(self):
        ...
        for m, F, c, asP, phsel, bg, msep, sf in zip(ms,Fs, cs, asPs, phsels, bg_table, max_sep, search_fused):
            ...
            start, stop = smc.burstsearch(self.origin.times, self.origin.dets,
                                            periods, bg['bg', phsel],
        ...
        if msep > 0:
            starts, stops = smc.fusebursts(starts, stops, msep)
    ...
    def param_preprocess(cls, params:Sequence[tuple[str,Any]]|tupledict, parents:dict[str:Param])->tuple[dict, dict]:
        ...
        detdef = bg[0].detdef # get detdef so proper conversion of PhSels to posiive can take place
...
class NphBG(ChildPhotonTable):
    ...
    Columns
    -------
        nph_bg : float, (ph_sel:Ph_sel, starttype:str, stoptype:str)
