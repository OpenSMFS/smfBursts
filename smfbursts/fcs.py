...
from .datamodel.utils import fjit, fnumba
from .datamodel.tables import Param, Column, GateGroup
from .photondata import PhotonData, PhotonDataList, PhotonDataS
...
def _expand_bursts(data:PhotonData, bursts:Param, streamT:PhSel=PhSel('all'), streamU:PhSel=PhSel('all'), 
    ...
    start, stop = smc.fusebursts(btable['start']-expand, btable['stop']+expand, fuse)
    start[start < 0] = 0
    istart, istop = smc.index_ranges(timesT, start, stop)
    ...
    if streamT != streamU or nanoTbin != nanoUbin or not _comp_nanorange(nanoTrng, nanoUrng):
        ...
        istart, istop = smc.index_ranges(timesU, start, stop)
