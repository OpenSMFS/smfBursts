...
class PhEventsRawDiskDict(MappedAttrDD, TypedValueDD):
    ...
    _mut:bool #: if data has been changed since initial creation

    def __init__(self, dct:None|dict=None, group:None|tb.Group=None):
        self._in_init = True
...
class PhGroupRaw(_DataLike):
    ...
    def _get_channel_sequence(self, spec:dict[str,np.ndarray[np.number]], rgx:re.Pattern,
                              max_skip:int, n:None|int=None)->np.ndarray[np.object_]:
...
class PhotonHDF5Data(_DataLike):
    ...
    __slots__ = ('photon_data', 'setup', 'acquisition_duration', 'description',
                 'sample', 'identity', 'provenance', 'filename', 'user', 'format_name', 'format_version',
    ...
    def _load_hdf5(cls, file:tb.File, ondisk:bool=False, track:bool=True, strict:bool=True)->"PhotonHDF5Data":
        ...
        photon_data = [PhGroupRaw.load_group(pharray, ondisk=ondisk) for pharray in photon_data]
        ...
        kwargs = dict()
        if 'acquisition_duration' in file.root:
            kwargs['acquisition_duration'] = file.root.acquisition_duration.read()
        for field in ('identity', 'provenance'):
            if field in file.root:
                kwargs[field] = dict_from_group(file.root[field])
        for field in ('description', 'filename'):
            if field in file.root:
                kwargs[field] = file.root[field].read()
        return cls(photon_data=photon_data, setup=setup, _file=file, _track=track, **kwargs)

    @classmethod
    def load_hdf5(cls, filename:str|PathLike, ondisk:bool=False, track:bool=True)->"PhotonHDF5Data":
        """
    ...
    def _save_group_future(self)->_GroupFuture:
        ...
        if 'user/smfBursts' in file.root:
            return _GroupFuture(self._file.root['user/smfBursts'], self)
        def create_group()->tb.Group:
            if 'user' not in self._file.root:
                file.create_group(self._file.root, 'user')
            if 'smfBursts' not in file.root.user:
                file.create_group(file.root.user, 'smfBursts')
            return file.root.user.smfBursts
    ...
    def save_photonHDF5(self, file:str|PathLike|tb.File=None, close:bool=None, **kwargs)->tb.File:
        """
        Save data as a photon-HDF5 file.
...
def _sort_tcspc_param(i:int, pd:PhGroupRaw, setup:SetupSpec, name:str,
    ...
    if name in setup.get('detectors', {}):
...
_setup_field_map = (('excitation_wavelengths', 'ex_wv'), ('excitation_polarizations', 'ex_pol'),
                    ...
                    ('detection_split_ch_ratios', 'split_ratio'), ('excitation_alternated','alternated'))


def _get_uspecs(setup:SetupSpec)->dict:
    """Create dictionary of universal specs from PhotonHDF5Data"""
    spec =  {outkey:setup[setupkey] for setupkey, outkey in _setup_field_map 
            if setupkey in setup and setup[setupkey] is not None}
    if 'excitation_cw' in setup:
        spec['pulsed'] = ~np.asarray(setup['excitation_cw'], dtype=np.bool_)
    return spec
...
def regularize_dets(raw:PhotonHDF5Data, alex_type:str=None, autosave:bool=False, 
                    keepraw:None|bool=False, group:GroupFuture=None, 
                    track:bool=True, load_saved:bool=True, asdatalist:bool=True, 
                    unpack:bool=True)->PhotonData|tuple[PhotonData,...]|PhotonDataList:
    """
    ...
    keepraw : None | bool
        If True, keep reference to raw object in returned object(s), this ensures
        that returned objects can create photon-HDF5 files. If False, then only
        a weakref to the original data is kept, thus a saving as photon-HDF5 is
        possible only as long as the raw remains alive (has a strong reference).
        The default is False.
    ...
    track : bool, optional
        ...
        The default is True
    load_saved : bool
        If :code:`True` then set group of returned data to ``user/smfBursts/photon_dataX``
        so that any previously saved columns can be loaded instead of calculated.
        The default is True.
    asdatalist : bool, optional
        If True, return as a PhotonDataList object, 
        unless unpack is True and single spot measurement, then unpack into PhotonData.
        If False, return is tuple of PhotonData, unless single spot and unpack is True,
        The default is True
    unpack : bool
        If :code:`True`, and single spot measurement, return a PhotonData object. 
        If :code:`False`, return will depend on value of ``asdatalist``, PhotonDataList if
        ``asdatalist`` is :code:`True`, or tuple of PhotonData if ``asdatalist`` is :code:`False`.
        The default is True.

    Returns
    -------
    PhotonDataList | PhotonData | tuple[PhotonData, ...]
    ...
    for i, pd in enumerate(raw.photon_data):
        ...
        sdict = dict(clk_p=pd.time_specs['timestamps_unit'])
        sdict.update(uspecs)
        if hasattr(pd, 'nanos'):
    ...
    if load_saved:
        saved_group = raw._save_group_future()
        if saved_group._created:
            for i, o in enumerate(out):
                o.load_all_from_group(saved_group[f'photon_data{i}'])
